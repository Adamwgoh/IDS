...
FrameFormat frameFormatDepth = FRAME_FORMAT_QQVGA; const int widthDepth = 160, heightDepth = 120; // Depth QVGA

FrameFormat frameFormatColor = FRAME_FORMAT_QQVGA; const int widthColor = 160, heightColor= 120; // Color VGA
...
void onNewDepthSample(DepthNode node, DepthNode::NewSampleReceivedData data)
{
	...
	printf("depth size is : %d\n", data.depthMap.size());
	printf("uvdepthmap size : %d\n", data.uvMap.size());
...
void configureNode(Node node)
{
    if ((node.is<DepthNode>())&&(!g_dnode.isSet()))
    {
		...
		printf("configuring depth node complete\n");
    }
    ...
    if ((node.is<ColorNode>())&&(!g_cnode.isSet()))
    {
        ...
        configureColorNode();
        g_context.registerNode(node);
		printf("configuring color node complete\n");
    }
